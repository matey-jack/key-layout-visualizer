...
To choose the appropriate Shift pairings, we simply check whether the keymap has an `ä` key:
if yes, it's German, otherwise English.
Sections and list items below carry the tags [English] and [German] where they apply to
keymaps of only one of the two character sets.
...
Next parts:
 ...
 - [English] Replace redundant Shift keys with Nav keys.
...
But to create a comfortable and effective key map, we should take into account that the Shift level is 
easier to access than the AltGr level – both physically because of the large and well-positioned Shift keys and mentally, 
because we are already used to using Shift for accessing punctuation during typing.
Therefore, we want prose punctuation characters – those that appear as part of text that users can type in a fast flow – 
to be mapped on the base and Shift levels.
...
- `-` next to `,` and `.`,
- breaking the `;:` character pair and using that home-finger position for a letter – or in the case of English, the apostrophe, 
  which is used inside words like a letter. This change is actually also an improvement in finger movement for typing English texts. 
...
While the ANSI English character map is the model for our English keymaps and
the Standard German (qwertz) is the model for the German ones,
the "base level only" keymaps take some liberties with both:
 ...
 - Finally, on some of the larger keyboards with a German keymap I map more characters than an ANSI keyboard has. 
...
With a German character set, whose three extra letters take up key positions, this goes down to only 4 to 7 punctuation keys left.
...
## [English] Which characters go where?

Let's first consider the keymaps with an English character set, which are modelled on the ANSI English keymap.
...
The seven pairs match the seven punctuation keys an English keymap has there:
`,` `.` `;` and one of `/` and `-` from the letter map, plus `+`, `'`, and the other one of `/` and `-`
...
Ironically, this generalizes to the German keymaps more easily than on keyboards with a number row:
...
The Ergoslat has only 7 punctuation keys with an English keymap,
which leaves only 4 punctuation keys for a German one.
...
In exchange, we only do this for the `ansi30` keymap, which is enough to show what the key levels can do.
