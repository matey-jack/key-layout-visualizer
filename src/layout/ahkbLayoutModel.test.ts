...
describe("ahkbLayoutModel angle mod", () => {
    ...
    it("shifts lower row letters and places delete between backslash and slash", () => {
        expect((angleModLayout.frameMappings[KeymapTypeId.Ansi30])![KeyboardRows.Lower]).toEqual(
            ["⇧", 1, 2, 3, 4, "\\", "⌦", 9, 5, 6, 7, 8, "⇧"],
        );
    });

    it("applies the same rotation to the thumb mapping", () => {
        expect((angleModLayout.frameMappings[KeymapTypeId.Thumb30])![KeyboardRows.Lower]).toEqual(
            ["⇧", 1, 2, 3, 4, "\\", "⌦", "/", 5, 6, 7, 8, "⇧"],
        );
    });
});
