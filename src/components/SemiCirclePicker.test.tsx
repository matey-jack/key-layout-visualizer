...
import { SemiCirclePicker } from './SemiCirclePicker.tsx';

describe('SemiCirclePicker', () => {
    describe('step=1', () => {
        it('renders all options as numeric labels when ≤9 options', () => {
            render(<SemiCirclePicker min={14} max={16} current={15} step={1} onChange={() => {}} />);
        ...
        it('calls onChange with the clicked value', () => {
            ...
            render(<SemiCirclePicker min={14} max={16} current={15} step={1} onChange={onChange} />);
        ...
        it('renders every other option as a dot when >9 options', () => {
            // min=1, max=11 step=1 → 11 options; idx 0,2,4,6,8,10 are labels, 1,3,5,7,9 are dots
            render(<SemiCirclePicker min={1} max={11} current={6} step={1} onChange={() => {}} />);
    ...
    describe('step=0.5', () => {
        it('shows whole numbers as labels and fractions as dots when >5 options', () => {
            ...
            render(<SemiCirclePicker min={13.5} max={16} current={14} step={0.5} onChange={() => {}} />);
        ...
        it('clicking a dot option calls onChange with the correct value', () => {
            const onChange = vi.fn();
            render(<SemiCirclePicker min={13.5} max={16} current={14} step={0.5} onChange={onChange} />);
        ...
        it('shows all options as numeric labels when ≤5 options', () => {
            ...
            render(<SemiCirclePicker min={0.5} max={2.5} current={1} step={0.5} onChange={() => {}} />);
        ...
        it('marks the current value as selected', () => {
            render(<SemiCirclePicker min={13.5} max={16} current={15} step={0.5} onChange={() => {}} />);
            expect(screen.getByRole('button', {name: '15'})).toHaveClass('selected');
            expect(screen.getByRole('button', {name: '14'})).not.toHaveClass('selected');
        });
    });
