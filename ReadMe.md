...
* The Semi-Ergo Explorer dynamically generates keyboard layouts based on some parameters. 
  ...
  Its entry point is `semi-ergo.html` and its code is isolated in the `src/semi-ergo/`.
...
## Semi-Ergo (Keyboard Layout) Explorer
...
keys in each row. 
It would be smaller without the number row, since that has one less stagger offset bringing keys to collide in the center.
